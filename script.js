const codeGuesses = Array.from(document.querySelectorAll(".guess-and-color-selection"));
...
class Guess {
    ...
    renderGuess() {
        ...
        const html = `<div data-id = "${pastGuesses.indexOf(this) + 1}"class="past-guess past-guess-${(pastGuesses.indexOf(this) + 1) % 2 === 0 ? 'even' : 'odd'}">
...
const gameInit = function () {
    const colors = ['blue', 'grey', 'yellow', 'red', 'orange', 'green', 'white', 'pink'];
    const code = [];
    for (let i = 0; i < 4; i++) {
...
const checkGuess = function (guess, code) {
    ...
    try {
        ...
        if (pastGuesses.some(pastGuess => pastGuess.getGuess().join('') === guess.join(''))) throw new Error('Error : Sequence has already been guessed');
        ...
        if (rightColorRightSpot < 4) {
            pastGuesses.push(new Guess([...guess], rightColorRightSpot, rightColorWrongSpot, wrongColor));
